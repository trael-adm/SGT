(function () {
    ...
    // Cópia adaptada de assets/js/producao-lista.js (LAB/IQF) — o modal de
    // reprova aqui fala com api/pintura-retornos-acao.php (`reprovar_pintura`,
    // já existente e testada), não com api/retrabalho-acao.php.
    var API          = window.PINTURA_LISTA_API || '';
    var PINTURA_API  = window.PINTURA_API || '';
    var REPROVAS     = window.PINTURA_REPROVAS || [];
    ...
    function postPinturaAcao(payload) { return postJson(PINTURA_API, payload); }
    ...
    document.addEventListener('click', function (e) {
        ...
        var d = btn.dataset;
        var idProjInput = document.getElementById('rep-form-id-projeto');
        var nsInput     = document.getElementById('rep-form-ns');

        if (idProjInput) idProjInput.value = d.id_projeto || '';
        if (nsInput) nsInput.value = d.ns_transformador || '';

        var nsDisplay = document.getElementById('rep-modal-ns');
        var projDisplay = document.getElementById('rep-modal-projeto');
    ...
    ['rep-modal-close', 'rep-modal-cancelar'].forEach(function (id) {
        var el = document.getElementById(id);
        if (el) el.addEventListener('click', closeModal);
    });
    modal.addEventListener('click', function (e) { if (e.target === modal) closeModal(); });

    // ─── Submeter (api/pintura-retornos-acao.php, ação reprovar_pintura) ───────────
    form.addEventListener('submit', function (e) {
        ...
        var obsEl       = document.getElementById('rep-obs');
        var observacoes = obsEl ? obsEl.value : '';

        var hiddenInputs = reprovasList.querySelectorAll('.rep-hidden-id');
        var idsReprova = [];
        ...
        .then(function (res) {
            if (res && res.sucesso) {
                notify(res.mensagem || 'Reprovação registrada e enviada para os Retornos da Pintura.', 'success');
    ...
    // ─── Excluir registro (lixeira) — api/pintura-acao.php ──────────────────────
    ...
    if (excluirModal) {
        ...
        if (excluirConfirmarBtn) {
            excluirConfirmarBtn.addEventListener('click', function () {
                ...
                postPinturaAcao({ acao: 'remover_etapa', ns_transformador: nsParaExcluir })
                    .then(function (res) {
                        if (res && res.sucesso) {
                            window.location.reload();
                        } else {
