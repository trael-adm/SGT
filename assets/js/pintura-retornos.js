(function () {
    ...
    // Mesmo padrão de interação de assets/js/producao-retornos.js (combobox,
    // modais de Aprovar/Reprovar/Excluir) — backend próprio (ver
    // api/pintura-retornos-acao.php), não compartilha nenhuma ação com LAB/IQF.
    var API = window.PINTURA_RETORNOS_API || '';
    var catalogo = window.PINTURA_RETORNOS_CATALOGO || [];
    var reprovasPorItem = window.PINTURA_RETORNOS_ITENS || {};
    ...
    try {
        localStorage.removeItem('sgt_pintura_retornos_expand_all');
    } catch (e) {}
    ...
    function setAllRows(expand) {
        ...
        document.querySelectorAll('.js-toggle-retorno').forEach(function (btn) {
            ...
            if (!row) return;
            if (expand) { row.classList.add('is-open'); btn.setAttribute('aria-expanded', 'true'); }
            else { row.classList.remove('is-open'); btn.setAttribute('aria-expanded', 'false'); }
        });
    }
        ...
        postAcao({ acao: 'aprovar_retorno', id_projeto: d.id_projeto, ns_transformador: d.ns_transformador })
            .then(function (res) {
                if (res && res.sucesso) {
                    ...
                    var err = (res && res.erro) || 'Erro ao aprovar retorno.';
                    if (typeof window.showAlert === 'function') window.showAlert(err, 'danger'); else alert(err);
                    btn.disabled = false;
                }
            })
            .catch(function () {
                if (typeof window.showAlert === 'function') window.showAlert('Falha de conexão.', 'danger'); else alert('Falha de conexão.');
                btn.disabled = false;
            });
    });
    ...
    function fecharMiniModal() { if (miniModal) miniModal.style.display = 'none'; }
    function fecharGrandeModal() {
        ...
        if (grandeSubmitBtn) { grandeSubmitBtn.disabled = false; grandeSubmitBtn.textContent = 'Confirmar Reprovação'; }
    }
    ...
    document.addEventListener('click', function (e) {
        ...
        var d = btn.dataset;
        itemPendente = {
            id_projeto: d.id_projeto, ns_transformador: d.ns_transformador,
            projeto_codigo: d.projeto_codigo, projeto_descricao: d.projeto_descricao, pedido_numero: d.pedido_numero
        };
    ...
    if (miniModal) miniModal.addEventListener('click', function (e) { if (e.target === miniModal) fecharMiniModal(); });
                ...
                if (reprovasAnteriores.length > 0) {
                    reprovasAnteriores.forEach(function (rep) {
                        adicionarLinhaReprova({
                            id: rep.id_reprova || rep.reprova_id,
    ...
    if (btnNovaReprova) {
        btnNovaReprova.addEventListener('click', function () {
            ...
            if (grandeList) { grandeList.innerHTML = ''; adicionarLinhaReprova(); }
            grandeModal.style.display = 'flex';
        });
    }
    ...
    if (grandeModal) grandeModal.addEventListener('click', function (e) { if (e.target === grandeModal) fecharGrandeModal(); });
    ...
    function configurarCombobox(comboboxEl, hiddenInput, onSelect) {
        ...
        function renderLista(termo) {
            ...
            if (filtrados.length === 0) {
                ...
                vazio.style.cssText = 'padding:8px 10px;font-size:12px;color:#9ca3af;';
        ...
        function abrirDropdown() { renderLista(input.value); dropdown.style.display = 'block'; comboboxEl.classList.add('is-open'); }
        function fecharDropdown() { dropdown.style.display = 'none'; comboboxEl.classList.remove('is-open'); }

        input.addEventListener('focus', abrirDropdown);
        input.addEventListener('input', function () { hiddenInput.value = ''; abrirDropdown(); });
        if (toggle) toggle.addEventListener('click', function (e) { e.stopPropagation(); if (dropdown.style.display === 'block') fecharDropdown(); else { input.focus(); abrirDropdown(); } });
        document.addEventListener('click', function (e) { if (!comboboxEl.contains(e.target)) fecharDropdown(); });
    }
    ...
    function adicionarLinhaReprova(itemPreenchido) {
        ...
        if (removeBtn) removeBtn.addEventListener('click', function () { block.remove(); atualizarBotoesRemover(); });
    ...
    if (grandeAddBtn) grandeAddBtn.addEventListener('click', function () { adicionarLinhaReprova(); });
    ...
    if (grandeForm) {
        grandeForm.addEventListener('submit', function (e) {
            ...
            var idReprovas = [];
            grandeList.querySelectorAll('.rep-reprova-select').forEach(function (input) { if (input.value) idReprovas.push(input.value); });

            if (idReprovas.length === 0) {
                if (grandeErro) { grandeErro.textContent = 'Selecione pelo menos um motivo de reprovação.'; grandeErro.style.display = 'block'; }
                return;
            }
            ...
            .then(function (res) {
                if (res && res.sucesso) {
                    ...
                    if (grandeErro) { grandeErro.textContent = (res && res.erro) || 'Erro ao registrar reprova no retorno.'; grandeErro.style.display = 'block'; }
                    grandeSubmitBtn.disabled = false;
                    grandeSubmitBtn.textContent = 'Confirmar Reprovação';
                }
            })
            .catch(function () {
                if (grandeErro) { grandeErro.textContent = 'Falha de comunicação com o servidor.'; grandeErro.style.display = 'block'; }
                grandeSubmitBtn.disabled = false;
                grandeSubmitBtn.textContent = 'Confirmar Reprovação';
            });
        });
    ...
    function fecharExcluirModal() {
        ...
        if (excluirConfirmarBtn) { excluirConfirmarBtn.disabled = false; excluirConfirmarBtn.textContent = 'Excluir Retorno'; }
    }
    ...
    if (excluirModal) {
        document.addEventListener('click', function (e) {
            ...
            if (!btn) return;
            var d = btn.dataset;
            itemExcluirPendente = { id_projeto: d.id_projeto, ns_transformador: d.ns_transformador };
        ...
        if (excluirConfirmarBtn) {
            excluirConfirmarBtn.addEventListener('click', function () {
                ...
                postAcao({ acao: 'excluir_retorno', id_projeto: itemExcluirPendente.id_projeto, ns_transformador: itemExcluirPendente.ns_transformador })
                    .then(function (res) {
                        if (res && res.sucesso) {
                            window.location.reload();
                        } else {
