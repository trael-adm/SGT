(function () {
    ...
    // Cópia adaptada de assets/js/producao.js (LAB/IQF) — backend próprio
    // (api/pintura-acao.php), nenhum código compartilhado.
    var API      = window.PINTURA_API || '';
    var ESTACAO  = window.PINTURA_ESTACAO || 'PIN';
    var ESTACAO_NOME = 'na Pintura';
    ...
    // ─── Elementos ────────────────────────────────────────────────────────────
    var pinCard   = document.querySelector('.station-card');
    var pinStatus = document.querySelector('.station-card__status');
    var pinSub    = document.querySelector('.station-card__sub');
    ...
    if (!pinCard || !fabBtn || !overlay) return; // página não carregou os elementos esperados
    ...
    function renderStationState(item) {
        // A peça vai direto para a aba Lista pra ser reprovada — o card não
        // precisa manter o item "em andamento" preso.
        if (currentItemId === 'vazio' && pinStatus.innerHTML !== '') return;
        currentItemId = 'vazio';
        ...
        pinStatus.innerHTML =
            ...
            '</div>';
        pinSub.textContent = 'Pronto para leitura';
        pinCard.classList.remove('is-active');
    }

    renderStationState(window.PINTURA_ITEM_ATUAL || null);
                ...
                if ('BarcodeDetector' in window) {
                    if (!window.globalBarcodeDetector) {
                        try {
                            ...
                            try {
                                window.globalBarcodeDetector = new BarcodeDetector();
                            } catch (e2) {
                    ...
                    if (window.globalBarcodeDetector) {
                        detectTimer = setInterval(scanFrameNative, 200);
    ...
    function confirmEntry() {
        ...
        var payload = { acao: 'confirmar', codigo: pendingResult.codigo, metodo_insercao: (pendingResult.origem === 'manual' ? 'manual' : 'scanner') };
