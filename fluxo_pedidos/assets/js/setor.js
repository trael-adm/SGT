...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function carregarSetor() {
        ...
        if (state.setor === 'PRODUCAO') {
            try {
                let url = `api.php?action=planilha_producao&status_fila=${state.statusFila}`;
            ...
            try {
                const res = await fetch(`api.php?action=setor_pedidos&setor=${encodeURIComponent(state.setor)}`);
                const data = await res.json();
    ...
    window.abrirModalPedido = async function(cdPedido) {
        ...
        try {
            const res = await fetch(`api.php?action=detalhe_pedido&cdPedido=${cdPedido}`);
            const data = await res.json();
