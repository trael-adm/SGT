...
$where  = ["pe.estacao = 'IQF'", "pe.status = 'aguardando_retorno'", "pe.deleted_at IS NULL"];
$params = [];
...
// Transformadores enviados de volta à Inspeção Final pela Triagem do Retrabalho
...
$mesesDisponiveis = $pdo->query("
    ...
    WHERE estacao = 'IQF' AND status = 'aguardando_retorno' AND deleted_at IS NULL
    ORDER BY ym DESC
...
if ($retornos) {
    ...
    $stmtRep = $pdo->prepare("
        ...
        INNER JOIN (
            ...
            FROM retrabalhos
            WHERE deleted_at IS NULL AND setores_destino IS NOT NULL AND FIND_IN_SET('inspecao_final', setores_destino)
        ...
        WHERE r.deleted_at IS NULL
          AND r.setores_destino IS NOT NULL AND FIND_IN_SET('inspecao_final', r.setores_destino)
...
// Catálogo de reprovas para o modal de Nova Reprova (apenas IQF e GER)
...
$pageTitle = 'Retornos à Inspeção Final';
...
<div style="margin-bottom:18px;">
    <h1 style="font-size:var(--font-size-xl,20px);font-weight:700;margin-top:2px;">Retornos à Inspeção Final</h1>
    <p class="text-secondary" style="font-size:13px;color:var(--color-text-secondary,#6b7280);margin-top:2px;">
        Transformadores encaminhados pelo Retrabalho, aguardando dar entrada de novo na Inspeção Final
    </p>
</div>
    ...
    <form method="GET" class="filter-bar">
        <div class="filter-group-left">
            ...
            <?php if ($temFiltroAtivo): ?>
                <a href="<?= htmlspecialchars($base) ?>/pages/inspecao_final/retornos.php" class="filter-btn filter-btn-clear" title="Limpar filtros">
