...
/**
 * Registro de Reprova (apontamento) da estação PIN (Pintura) — cópia
 * adaptada de pages/producao/index.php (mesmo padrão de LAB/IQF), falando
 * com um backend próprio e isolado (api/pintura-acao.php).
 */
...
$canEdit = podeEditar('pin.reg') || isAdmin();

$base = defined('APP_URL') ? APP_URL : '';

if (!hasAcesso('pin.reg')) {
    if (hasAcesso('pin.lis')) {
        header('Location: ' . $base . '/pages/pintura/lista.php');
        exit;
    } elseif (hasAcesso('pin.retornos')) {
        header('Location: ' . $base . '/pages/pintura/retornos.php');
        exit;
    }
}
...
// Item em andamento na Pintura agora — carregado direto do banco (estado real,
...
$itemAtualPintura = $stmt->fetch() ?: null;
if ($itemAtualPintura) {
    $itemAtualPintura['data_inicio'] = isoComOffset($itemAtualPintura['data_inicio']);
}

$pageTitle = 'Registro de Reprova — Pintura';
...
<div class="station-grid">
    ...
    <section class="station-card" id="pinCard">
        <header class="station-card__head">
            <span class="station-pill" style="background:#fdf2f8;color:#be185d;">PIN</span>
            <div>
                <h2>Pintura</h2>
                <p class="station-card__sub" id="pinSub">Nenhum transformador em andamento</p>
            </div>
        </header>

        <div class="station-card__status" id="pinStatus"></div>
...
<script>
    window.PINTURA_API = <?= json_encode($base . '/api/pintura-acao.php', JSON_HEX_TAG | JSON_HEX_AMP) ?>;
    window.PINTURA_ESTACAO = <?= json_encode($estacaoAtual, JSON_HEX_TAG | JSON_HEX_AMP) ?>;
    window.PINTURA_ITEM_ATUAL = <?= json_encode($itemAtualPintura, JSON_HEX_TAG | JSON_HEX_AMP | JSON_UNESCAPED_UNICODE) ?>;
</script>
<script src="<?= htmlspecialchars($base) ?>/assets/js/vendor/jsQR.js"></script>
<?php $pinJsVer = @filemtime(__DIR__ . '/../../assets/js/pintura-apontamento.js') ?: (defined('APP_VERSION') ? APP_VERSION : '1'); ?>
<script src="<?= htmlspecialchars($base) ?>/assets/js/pintura-apontamento.js?v=<?= htmlspecialchars((string) $pinJsVer) ?>"></script>
