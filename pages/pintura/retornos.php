...
/**
 * Fila de "Retornos da Pintura" — mesmo padrão visual/estrutural de
 * pages/producao/retornos.php, mas consultando a estação PIN (ver
 * _inicial/migrar-pintura-estacao-pin.sql) e falando com um backend
 * dedicado e isolado (api/pintura-retornos-acao.php), sem nenhuma função
 * compartilhada com o fluxo de LAB/IQF.
 */
...
requireLogin();
if (!hasAcesso('tab:pintura') && !hasAcesso('pin.retornos') && !hasAcesso('admin')) {
    requireAcessoModulo('tab:pintura');
}

$canEdit = podeEditar('pin.retornos') || isAdmin();
...
$where  = [
    "pe.estacao = 'PIN'",
    "pe.status = 'aguardando_retorno'",
    "pe.deleted_at IS NULL",
    // Isolamento: só entra na fila da Pintura o que de fato foi reprovado pela Pintura.
    "EXISTS (
        SELECT 1 FROM retrabalhos r_orig
        WHERE r_orig.ns_transformador = pe.ns_transformador
          AND r_orig.id_projeto = pe.id_projeto
          AND r_orig.deleted_at IS NULL
          AND r_orig.estacao = 'PIN'
    )"
];
$params = [];
...
$mesesDisponiveis = $pdo->query("
    ...
    FROM producao_etapas pe
    WHERE pe.estacao = 'PIN' AND pe.status = 'aguardando_retorno' AND pe.deleted_at IS NULL
      AND EXISTS (
          SELECT 1 FROM retrabalhos r_orig
          WHERE r_orig.ns_transformador = pe.ns_transformador
            AND r_orig.id_projeto = pe.id_projeto
...
// Reprovas que motivaram cada retorno (apenas da estação PIN)
$reprovasPorItem = [];
if ($retornos) {
...
// Catálogo de reprovas pro modal de Nova Reprova — mesmo filtro usado pelo
// modal "Reprovar" do Paint Check (ver pages/qualidade/paint-check.php).
...
function pinRetFmtDataHora(?string $iso): string
{
    if (!$iso) return '—';
    $ts = strtotime($iso);
...
function pinRetFmtData(?string $iso): string
{
    if (!$iso) return '—';
    $ts = strtotime($iso);
...
<style>
    ...
    /* Combobox de busca de reprovas — mesmo padrão de pages/producao/retornos.php */
...
<div class="page-fixed-layout">
    ...
    <div class="page-fixed-header">
        <h1 style="font-size:var(--font-size-xl,20px);font-weight:700;margin-top:2px;">Retornos da Pintura</h1>
        <p class="text-secondary" style="font-size:13px;color:var(--color-text-secondary,#6b7280);margin-top:2px;">
            Transformadores reprovados pelo Paint Check, aguardando correção e nova validação
        </p>
    </div>
    ...
    <form method="GET" class="filter-bar">
        <div class="filter-group-left">
            ...
            <?php if ($temFiltroAtivo): ?>
                <a href="<?= htmlspecialchars($base) ?>/pages/pintura/retornos.php" class="filter-btn filter-btn-clear" title="Limpar filtros">
    ...
    <?php if (!$retornos): ?>
        <p style="text-align:center;padding:36px 16px;color:var(--color-text-muted);font-size:13px;">
            <?= $temFiltroAtivo ? 'Nenhum transformador encontrado para os filtros selecionados.' : 'Nenhum transformador aguardando retorno da Pintura.' ?>
        </p>
    <?php else: ?>
                ...
                <tbody>
                    ...
                    ?>
                        <tr>
                            ...
                            <td><span class="font-mono font-600"><?= htmlspecialchars($r['projeto_codigo'] ?? '—') ?></td>
                            ...
                            <td><?= htmlspecialchars(pinRetFmtDataHora($r['data_inicio'])) ?></td>
                                            ...
                                            <tbody>
                                                <?php foreach ($reprovas as $rep): ?>
                                                    <tr>
                                                        ...
                                                        <td><?= htmlspecialchars(pinRetFmtData($rep['data_reprova'])) ?></td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            </tbody>
                                        </table>
...
<script>
    window.PINTURA_RETORNOS_API = <?= json_encode($base . '/api/pintura-retornos-acao.php') ?>;
    window.PINTURA_RETORNOS_CATALOGO = <?= json_encode($reprovasCatalogo, JSON_HEX_TAG | JSON_HEX_AMP) ?>;
    window.PINTURA_RETORNOS_ITENS = <?= json_encode($reprovasPorItem, JSON_HEX_TAG | JSON_HEX_AMP) ?>;
</script>
<?php $pinRetJsVer = @filemtime(__DIR__ . '/../../assets/js/pintura-retornos.js') ?: (defined('APP_VERSION') ? APP_VERSION : '1'); ?>
<script src="<?= htmlspecialchars($base) ?>/assets/js/pintura-retornos.js?v=<?= htmlspecialchars((string) $pinRetJsVer) ?>"></script>
