...
requireLogin();
if (!hasAcesso('tab:pintura') && !hasAcesso('pin.ret') && !hasAcesso('admin')) {
...
// ─── Condição: Apenas reprovas de PINTURA, SERIGRAFIA e CAMADA (ou enviadas à Pintura) ─
$where  = [
    'r.deleted_at IS NULL',
    "(rep.familia IN ('PINTURA', 'SERIGRAFIA', 'CAMADA') OR r.setores_destino LIKE '%pintura%')",
    "NOT EXISTS (
...
$reprovas = $pdo->query("SELECT id, codigo, familia, descricao, local FROM reprovas WHERE ativo = 1 AND familia IN ('PINTURA', 'SERIGRAFIA', 'CAMADA') ORDER BY ordem, LENGTH(codigo), codigo")->fetchAll();

$familiasDisponiveis = ['PINTURA', 'SERIGRAFIA', 'CAMADA'];
...
// ─── Helpers de exibição ──────────────────────────────────────────────────────
$statusMap = [
    'agu_abertura'   => ['label' => 'Agu. Correção',   'bg' => '#fef2f2', 'fg' => '#dc2626'],
...
function rtUrl(array $overrides = []): string
{
    ...
    return htmlspecialchars($base . '/pages/pintura/relacao.php' . ($qs !== '' ? '?' . $qs : ''));
}
...
$pageTitle = 'Pintura — Relação de Retrabalhos';
...
<div class="page-fixed-layout">
    ...
    <div class="page-fixed-header">
        <h1 style="font-size:var(--font-size-xl,20px);font-weight:700;margin-top:2px;">Pintura — Relação de Retrabalhos</h1>
        <p class="text-secondary" style="font-size:13px;color:var(--color-text-secondary,#6b7280);margin-top:2px;">
            Listagem detalhada dos retrabalhos de Pintura, Serigrafia e Camada
        </p>
    </div>
    ...
    <form method="GET" class="filter-bar" id="rt-filtros">
        ...
        <div class="filter-group-left">
            ...
            <?php if ($temFiltroAtivo): ?>
                <a href="<?= htmlspecialchars($base) ?>/pages/pintura/relacao.php" class="filter-btn filter-btn-clear" title="Limpar filtros">
    ...
    <div style="overflow-x:auto;" id="rt-table-wrap">
        <table class="rt-table" style="min-width:1400px;">
            ...
            <tbody>
                <?php if (!$gruposPagina): ?>
                    <tr><td colspan="13"><div class="rt-empty">Nenhum retrabalho de pintura encontrado para os filtros selecionados.</div></td></tr>
    ...
    <div class="modal-box">
        <form id="rt-form">
            <div class="modal-head">
                <h2 id="rt-modal-title">Registrar retrabalho de pintura</h2>
            ...
            <div class="modal-body">
                ...
                <div class="rt-form-grid">
                    ...
                    <div class="rt-section">Reprova / contenção (Pintura / Serigrafia)</div>
                    <div class="rt-field">
...
(function() {
    ...
    try {
        localStorage.removeItem('sgt_pintura_expand_all');
        localStorage.removeItem('sgt_pintura_open_rows');
    } catch (e) {}
